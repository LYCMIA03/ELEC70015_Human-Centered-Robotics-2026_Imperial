...
# Ensure last target expires so retreat is not immediately interrupted.
sleep 2
...
# Keep target stream stopped so cooldown->retreat is tested without re-lock.
sleep 2
...
def first_idx(substr, start=0):
    for i in range(start, len(lines)):
        if substr in lines[i]:
            return i
    return None

def has_after(substr, start_idx=0, end_idx=None):
...
idx_false = first_idx("/trash_action received: False")
idx_true = first_idx("/trash_action received: True")

caseA_wait_from_log = has_after("REACHED -> WAITING_ACTION", 0, idx_false if idx_false is not None else None)
caseA_retreat_from_log = has_after("WAITING_ACTION -> RETREATING", idx_false, idx_true if idx_true is not None else None)
caseB_wait_from_log = has_after("REACHED -> WAITING_ACTION", idx_true if idx_true is not None else 0, None)
caseB_cool_from_log = has_after("WAITING_ACTION -> POST_ACCEPT_COOLDOWN", idx_true, None)
caseB_retreat_from_log = has_after("POST_ACCEPT_COOLDOWN -> RETREATING", idx_true, None)

# Ignore the expected traceback on forced shutdown (ROSInterruptException).
traceback_found = False
for m in re.finditer(r"Traceback \(most recent call last\):", log_text):
    tail = log_text[m.start():m.start() + 800]
    if "ROSInterruptException: ROS shutdown request" in tail:
        continue
...
out = {
    ...
    "caseA_waiting_action_observed": bool(a_wait) or caseA_wait_from_log,
    "caseA_retreat_after_action_false": bool(a_retreat) or caseA_retreat_from_log,
    "caseB_tracking_or_reached_observed": bool(b_track),
    "caseB_waiting_action_observed": bool(b_wait) or caseB_wait_from_log,
    "caseB_post_accept_cooldown_observed": bool(b_cool) or caseB_cool_from_log,
    "caseB_retreat_after_action_true": bool(b_retreat) or caseB_retreat_from_log,
