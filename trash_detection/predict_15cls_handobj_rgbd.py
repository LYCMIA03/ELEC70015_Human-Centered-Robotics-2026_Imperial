# -*- coding: utf-8 -*-
"""
15 类垃圾 + Hand-Object 双模型 RGB-D（仅 ONNX）：用深度返回最近目标 XYZ。

- 管线 1：15 类垃圾 + YOLOv8 person → 离最近垃圾最近的人 (person_near_waste_xyz)
- 管线 2：Hand-Object → 最近拿着物体的人 (holding_xyz)
- 输出：在「最近持物人」与「离垃圾最近的人」中选离相机更近的作为 target_xyz（可 UDP 发送）
- 15cls 与 handobj 仅允许 ONNX，无 .pt；person 仍为 yolov8m.pt。

相机与深度：Orbbec pyorbbecsdk。
依赖：pip install ultralytics onnxruntime-gpu opencv-python pyorbbecsdk（Jetson 可用 pypi.jetson-ai-lab.io/jp6/cu126）
运行：
  python trash_detection/predict_15cls_handobj_rgbd.py
  python trash_detection/predict_15cls_handobj_rgbd.py --stream-enable --onnx-ep cuda
  # 人检测也用 ONNX（减轻显存/CUBLAS 冲突）：先导出 person ONNX，再指定
  # yolo export model=yolov8m.pt format=onnx imgsz=640  # 得到 yolov8m.onnx，放到 onnx/ 并命名为 yolov8m_640.onnx
  python trash_detection/predict_15cls_handobj_rgbd.py --stream-enable --weights-person onnx/yolov8m_640.onnx
  # 完全不用 PyTorch（三模型全 ONNX）：用 predict_15cls_handobj_rgbd_onnx_only.py
"""
import argparse
import gc
import json
...
ONNX_DIR = TRASH_ROOT / "onnx"
WEIGHTS_15CLS_NEW_ONNX = TRASH_ROOT / "weight_new" / "best.onnx"
WEIGHTS_15CLS_ONNX = ONNX_DIR / "15cls_640.onnx"
WEIGHTS_HANDOBJ_ONNX = ONNX_DIR / "handobj_640.onnx"
WEIGHTS_PERSON = "yolov8m.pt"
WEIGHTS_PERSON_ONNX = ONNX_DIR / "yolov8m_640.onnx"  # 可选：person 也用 ONNX 时用，避免 PyTorch 占显存
...
def _install_onnxruntime_trt_first(onnx_ep="trt", trt_workspace_bytes=None):
    if trt_workspace_bytes is None:
        trt_workspace_bytes = 64 * 1024 * 1024
    if onnx_ep not in ("trt", "cuda", "cpu"):
        return
    try:
    ...
    def _session(path_or_bytes, *args, **kwargs):
        prov = kwargs.get("providers")
        if prov is None:
            return _orig(path_or_bytes, *args, **kwargs)
        dev_id = 0
        if not prov or not (isinstance(prov[0], tuple) and prov[0][0] == "CUDAExecutionProvider"):
            return _orig(path_or_bytes, *args, **kwargs)
        dev_id = prov[0][1].get("device_id", 0)
        avail = ort.get_available_providers()
        kwargs = dict(kwargs)
        if onnx_ep == "cpu":
            kwargs["providers"] = ["CPUExecutionProvider"]
            return _orig(path_or_bytes, *args, **kwargs)
        cuda_pair = ("CUDAExecutionProvider", {"device_id": dev_id})
        cpu_only = ["CPUExecutionProvider"]
        if onnx_ep == "cuda":
            attempts = [[cuda_pair, "CPUExecutionProvider"]] if "CUDAExecutionProvider" in avail else [cpu_only]
        else:
            import tempfile
            cache_dir = os.path.join(tempfile.gettempdir(), "ort_trt_cache")
            ...
            trt_opts = {
                ...
                "trt_max_workspace_size": str(int(trt_workspace_bytes)),
            }
            attempts = []
            if "TensorrtExecutionProvider" in avail:
                attempts.append([("TensorrtExecutionProvider", trt_opts), cuda_pair, "CPUExecutionProvider"])
            if "CUDAExecutionProvider" in avail:
                attempts.append([cuda_pair, "CPUExecutionProvider"])
            attempts.append(cpu_only)
        last_err = None
        for pl in attempts:
            try:
                kwargs["providers"] = pl
                ep0 = pl[0][0] if isinstance(pl[0], tuple) else pl[0]
                stop_hb = [False]
                ...
                def _trt_heartbeat():
                    ...
                    while not stop_hb[0]:
                        ...
                        sec += 30
                        print(
                            "  … TensorRT 首次编译较慢，已等待 %d 秒（可改 --onnx-ep cuda 跳过）…" % sec,
                            flush=True,
                        )

                hb = None
                if ep0 == "TensorrtExecutionProvider":
                    print("  → TensorRT 编译引擎中（首次可能 10～40 分钟）…", flush=True)
                    hb = threading.Thread(target=_trt_heartbeat, daemon=True)
                    hb.start()
                try:
                    sess = _orig(path_or_bytes, *args, **kwargs)
                finally:
                    stop_hb[0] = True
                print("ONNX Runtime 已用: %s" % ep0, flush=True)
                return sess
            except Exception as e:
                last_err = e
                ep = pl[0][0] if isinstance(pl[0], tuple) else pl[0]
                print("ONNX Runtime 尝试 %s 失败: %s" % (ep, e), flush=True)
        raise last_err
...
def main():
    p = argparse.ArgumentParser(description="15cls + Hand-Object RGB-D（ONNX + person pt）")
    p.add_argument("--weights-15cls", default=None)
    p.add_argument(
        "--weights-person",
        default=None,
        help="人检测权重：.pt 或 .onnx。不设时优先 onnx/yolov8m_640.onnx，否则 yolov8m.pt。person 用 ONNX 可减轻显存与 CUBLAS 冲突",
    )
    ...
    p.add_argument("--onnx-ep", choices=["trt", "cuda", "cpu"], default="trt")
    p.add_argument("--trt-workspace-mb", type=int, default=64)
    args = p.parse_args()

    yolo_device = "cuda" if torch.cuda.is_available() else "cpu"
    if args.device != "auto":
        yolo_device = args.device
    print("推理设备:", yolo_device)

    if args.weights_15cls:
        w15 = Path(args.weights_15cls)
        if not w15.is_absolute():
            w15 = TRASH_ROOT / w15
        if w15.suffix.lower() != ".onnx":
            print("15 类权重必须为 .onnx")
            return 1
    else:
        w15 = WEIGHTS_15CLS_NEW_ONNX if WEIGHTS_15CLS_NEW_ONNX.exists() else WEIGHTS_15CLS_ONNX
    if not w15.exists():
        print("未找到 15 类 ONNX")
        return 1
    ...
    if args.weights_handobj:
        ...
        if not whand.is_absolute():
            whand = (WEIGHTS_HANDOBJ_DIR / whand) if (WEIGHTS_HANDOBJ_DIR / whand).exists() else (ROOT.parent / whand)
        if whand.suffix.lower() != ".onnx":
            print("Hand-Object 必须为 .onnx")
            return 1
    else:
        whand = WEIGHTS_HANDOBJ_ONNX
    if not whand.exists():
        print("未找到 Hand-Object ONNX")
        return 1

    if args.weights_person is None:
        wp = WEIGHTS_PERSON_ONNX if WEIGHTS_PERSON_ONNX.exists() else WEIGHTS_PERSON
    else:
        wp = Path(args.weights_person)
        if not wp.is_absolute():
            wp = TRASH_ROOT / wp if (TRASH_ROOT / wp).exists() else Path(args.weights_person)
    if str(wp).endswith(".onnx") and not Path(wp).exists():
        print("未找到 person ONNX:", wp, "可先导出: yolo export model=yolov8m.pt format=onnx imgsz=640")
        return 1
    weights_person = str(wp)

    trt_ws = max(8, int(args.trt_workspace_mb)) * 1024 * 1024
    if yolo_device == "cuda":
        _install_onnxruntime_trt_first(args.onnx_ep, trt_workspace_bytes=trt_ws)
    elif args.onnx_ep != "trt":
        _install_onnxruntime_trt_first(args.onnx_ep, trt_workspace_bytes=trt_ws)

    model_waste = YOLO(str(w15))
    print("15cls 已加载(ONNX):", w15)
    model_handobj = YOLO(str(whand))
    print("Hand-Object 已加载(ONNX):", whand)
    if yolo_device == "cuda" and torch.cuda.is_available():
        torch.cuda.empty_cache()
        gc.collect()
    if args.onnx_ep == "trt" and yolo_device == "cuda":
        print(
            "【提示】TensorRT 首次编译很慢；要快请加 --onnx-ep cuda",
            flush=True,
        )
    print("正在初始化 ONNX（15cls）…", flush=True)
    names_15 = getattr(model_waste, "names", None)
    if yolo_device == "cuda" and torch.cuda.is_available():
        torch.cuda.empty_cache()
        gc.collect()
    print("正在初始化 ONNX（handobj）…", flush=True)
    _ = getattr(model_handobj, "names", None)
    model_person = YOLO(weights_person)
    print("Person:", weights_person)
    ...
    print("三模型: 15cls(0-%d) + person + handobj" % (NUM_WASTE_CLASSES - 1))
    ...
    if args.udp_enable:
        sender = UdpTargetSender(
            ...
            "combined_15cls_handobj_rgbd", args.udp_rate,
        )
    ...
    if not headless:
        cv2.namedWindow("15cls+HandObj RGB-D", cv2.WINDOW_NORMAL)
    ...
    try:
        while True:
            ...
            if not headless:
                if need_vis:
                    cv2.imshow("15cls+HandObj RGB-D", vis)
